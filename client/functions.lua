...
function RS.Functions.GetPlayerData(cb)
    if not cb then return RS.PlayerData end
    cb(RS.PlayerData)
end
...
function RS.Functions.HasItem(items, amount)
    return exports['redux-inventory']:HasItem(items, amount)
end

-- Utility

function RS.Functions.DrawText(x, y, width, height, scale, r, g, b, a, text)
...
function RS.Functions.DrawText3D(x, y, z, text)
    -- Use local function instead
...
RS.Functions.RequestAnimDict = lib.requestAnimDict

RS.Functions.LoadModel = lib.requestModel

RS.Functions.LoadAnimSet = lib.requestAnimSet

function RS.Functions.PlayAnim(animDict, animName, upperbodyOnly, duration)
...
function RS.Functions.NotifyV2(props)
    props.style = nil
    if not props.position then
        props.position = RS.NotifyPosition
    end
    lib.notify(props)
end

---@deprecated in favor of RS.Functions.NotifyV2()
...
function RS.Functions.Notify(text, notifyType, duration)

    TriggerEvent('DoLongHudText', text, duration, notifyType)
    --[[ notifyType = notifyType or 'inform'
    if notifyType == 'primary' then notifyType = 'inform' end
    duration = duration or 5000
    local position = RS.NotifyPosition
...
function RS.Debug(resource, obj, depth)
    TriggerServerEvent('RS:DebugSomething', resource, obj, depth)
end

-- Callback Functions --

-- Client Callback
function RS.Functions.CreateClientCallback(name, cb)
    RS.ClientCallbacks[name] = cb
end

function RS.Functions.TriggerClientCallback(name, cb, ...)
    if not RS.ClientCallbacks[name] then return end
    RS.ClientCallbacks[name](cb, ...)
end

-- Server Callback
function RS.Functions.TriggerCallback(name, cb, ...)
    RS.ServerCallbacks[name] = cb
    TriggerServerEvent('RS:Server:TriggerCallback', name, ...)
end
...
function RS.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    if lib.progressActive() then 
        RS.Functions.Notify("You are already doing something", "error", 5000)
        return
    end
...
function RS.Functions.GetVehicles()
    return GetGamePool('CVehicle')
end

function RS.Functions.GetObjects()
...
function RS.Functions.GetPeds(ignoreList)
...
function RS.Functions.GetClosestPed(coords, ignoreList)
    if coords then
    ...
    local peds = RS.Functions.GetPeds(ignoreList)
...
function RS.Functions.IsWearingGloves()
    local armIndex = GetPedDrawableVariation(cache.ped, 3)
    local model = GetEntityModel(cache.ped)
    if model == `mp_m_freemode_01` then
        if RS.Shared.MaleNoGloves[armIndex] then
            return false
        end
    else
        if RS.Shared.FemaleNoGloves[armIndex] then
            return false
        end
    end
    return true
...
function RS.Functions.GetClosestPlayer(coords)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(cache.ped)
    end
    local closestPlayers = RS.Functions.GetPlayersFromCoords(coords)
    local closestDistance = -1
...
function RS.Functions.GetPlayersFromCoords(coords, distance)
...
function RS.Functions.GetClosestVehicle(coords)
...
function RS.Functions.GetClosestObject(coords)
...
function RS.Functions.GetClosestBone(entity, list)
...
function RS.Functions.GetBoneDistance(entity, boneType, boneIndex)
    local bone
    if boneType == 1 then
...
function RS.Functions.AttachProp(ped, model, boneId, x, y, z, xR, yR, zR, vertex)
...
function RS.Functions.SpawnVehicle(model, cb, coords, isnetworked, teleportInto)
...
function RS.Functions.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function RS.Functions.GetPlate(vehicle)
    if vehicle == 0 then return end
    return RS.Shared.Trim(GetVehicleNumberPlateText(vehicle))
end

function RS.Functions.GetVehicleLabel(vehicle)
    if vehicle == nil or vehicle == 0 then return end
    return GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
end

function RS.Functions.SpawnClear(coords, radius)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(cache.ped)
...
function RS.Functions.GetVehicleProperties(vehicle)
...
function RS.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
        -- set vehicle stuff here
    end
end

function RS.Functions.LoadParticleDictionary(dictionary)
...
function RS.Functions.StartParticleAtCoord(dict, ptName, looped, coords, rot, scale, alpha, color, duration)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(cache.ped)
    end
    RS.Functions.LoadParticleDictionary(dict)
...
function RS.Functions.StartParticleOnEntity(dict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration)
    RS.Functions.LoadParticleDictionary(dict)
    UseParticleFxAssetNextCall(dict)
...
function RS.Functions.GetStreetNametAtCoords(coords)
...
function RS.Functions.GetZoneAtCoords(coords)
    return GetLabelText(GetNameOfZone(coords))
end

function RS.Functions.GetCardinalDirection(entity)
...
function RS.Functions.GetCurrentTime()
    local obj = {}
...
function RS.Functions.GetGroundZCoord(coords)
